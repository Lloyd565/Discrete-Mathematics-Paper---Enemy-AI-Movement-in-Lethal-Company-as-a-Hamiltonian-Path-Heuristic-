...
public abstract class EnemyAI : NetworkBehaviour
{
	...
	private IEnumerator CurrentSearchCoroutine()
	{
		...
		while (searchCoroutine != null && base.IsOwner)
		{
			...
			// inside CurrentSearchCoroutine while, after SetDestinationToPosition called
	...
	private IEnumerator ChooseNextNodeInSearchRoutine()
	{
		...
		for (int i2 = currentSearch.unsearchedNodes.Count - 1; i2 >= 0; i2--)
		{
			...
			else if (agent.isOnNavMesh && PathIsIntersectedByLineOfSight(currentSearch.unsearchedNodes[i2].transform.position, currentSearch.startedSearchAtSelf, avoidLineOfSight: false))
			{
				EliminateNodeFromSearch(i2);
			}
			else if (currentSearch.onlySearchNodesInLOS && Physics.Linecast(currentSearch.unsearchedNodes[i2].transform.position, currentSearch.currentSearchStartPosition, StartOfRound.Instance.collidersAndRoomMaskAndDefault, QueryTriggerInteraction.Ignore))
			...
			{
				// in ChooseNextNodeInSearchRoutine()
				if (!currentSearch.startedSearchAtSelf)
				{
					GetPathDistance(currentSearch.unsearchedNodes[i2].transform.position, currentSearch.currentSearchStartPosition);
				}
				if (pathDistance < closestDist && (!currentSearch.randomized || !gotNode || searchRoutineRandom.Next(0, 100) < 65))
				{
					closestDist = pathDistance; //[2]
	...
	public Transform ChooseFarthestNodeFromPosition(Vector3 pos, bool avoidLineOfSight = false, int offset = 0, bool doAsync = false, int maxAsyncIterations = 50, bool capDistance = false)
	{
		...
		for (int num2 = num; num2 < nodesTempArray.Length; num2++)
		{
			...
			if ((!capDistance || !(Vector3.Distance(base.transform.position, nodesTempArray[num2].transform.position) > 60f)) && !PathIsIntersectedByLineOfSight(nodesTempArray[num2].transform.position, calculatePathDistance: false, avoidLineOfSight))
			{
